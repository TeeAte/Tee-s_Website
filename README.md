...
- **动静分离的解耦设计**：
  - `worker.js`：核心路由拦截与 API 处理分发。
  - `frontend.js`：前台游客展示页面（首页动态、关于页面、留言板）。
  - `admin.js`：功能强大的管理后台面板（数据看板、动态管理、留言管理、图床管理、系统设置）。
  - `utils.js`：全局公用工具库（权限校验、XSS 防护等）。
...
- **Database (数据库)**: Cloudflare D1 (SQLite) -> 存储动态(Moments)、留言(Guestbook) 和 系统设置(Settings)
...
工具会自动使用 `esbuild` 将四个独立模块合并为 `dist/worker.js`，然后通过 `wrangler` 部署到你的 Cloudflare 账户（配置文件见 `wrangler.toml`）。
...
├── README.md             # 本文档
├── tools_and_tests/      # [存档] 过去迭代时的测试脚本及旧版代码备份
├── utils.js              # 全局通用方法库（比如 escapeHtml）
...
- **个人图床**：自带简单的 KV 图片直传与托管功能，动态发布不再受限于第三方图床。
- **留言板管理**：支持前台无刷新提交留言，管理员后台一键查看、编辑、删除违规留言。
